...
d3.csv("/homework/02-21/data/Somerville_Happiness_Survey_Responses.csv").then(function(data) {
    ...
    */
    let years_to_include = ['2013', '2015', '2017', '2019'];


    let filtered_data = data.filter(function(d) {

        return years_to_include.includes(d.Year) && +d.generalSatisfaction <= 10 && d.generalSatisfaction >= 0
        && d.annualIncome !== '999';

    });

    const grouped_data = d3.group(filtered_data, d => d.annualIncome);
    const averages = Array.from(grouped_data, ([key, value]) => ({
    annualIncome: key,
    generalSatisfaction: d3.mean(value, d => d.generalSatisfaction)
    }));
    ...
    */
    ...
    const genSat = {
        ...
        min: d3.min(averages, function(d) { return d.generalSatisfaction; }),
        max: d3.max(averages, function(d) { return d.generalSatisfaction; })
    ...
    */
    ...
    const xScale = d3.scaleBand()
        .domain(['Less than $10,000', '$10,000 to $24,999', '$25,000 to $49,999', '$50,000 to $74,999', '$75,000 to $99,999', '$100,000 to $149,999', '$150,000 to 200,000', '$200,000 or more'])
    ...
    */
    ...
    const points = svg.selectAll("rect")
        ...
        .enter()
        .append("rect")
            .attr("x", function(d) { return xScale(d.annualIncome); })
            .attr("y", function(d) { return yScale(d.generalSatisfaction); })
            .attr("width", xScale.bandwidth())
            .attr("height", function(d) { return height - (margin.bottom + yScale(d.generalSatisfaction)) })
            .attr("fill", "orange");
